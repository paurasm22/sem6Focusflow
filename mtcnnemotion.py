...
while True:
    ...
    try:
        ...
        for (x, y, w, h) in faces:
            ...
            # result = DeepFace.analyze(rgb_frame, actions=["emotion"], detector_backend="dlib")
            # result = DeepFace.analyze(face_region, actions=["emotion"], model_name="VGG-Face")
            # result = DeepFace.analyze(rgb_frame, actions=["emotion"], detector_backend="dlib")
            result = DeepFace.analyze(rgb_frame, actions=["emotion"], detector_backend="mtcnn")
            # result = DeepFace.analyze(rgb_frame, actions=["emotion"], detector_backend="retinaface")
            # result = DeepFace.analyze(rgb_frame, actions=["emotion"], detector_backend="ssd")
            # result = DeepFace.analyze(face_region, actions=["emotion"], detector_backend="mtcnn")
