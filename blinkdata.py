...
while cap.isOpened():
    ...
    if elapsed_time >= video_duration:
        # Classify the concentration level
        if blink_count < 10:
        ...
        cv2.putText(frame, f"Concentration: {concentration_level}", (30, 110),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

        # End the video capture after 1 minute
        cv2.putText(frame, "End of Session", (30, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        cv2.imshow("Blink Counter", frame)
        cv2.waitKey(2000)  # Display for 2 seconds before closing
        
        # Print the results at the end of the session
